...
func TestGenerateCreatesOneCaseAndReviewForPrimaryCycleOnly(t *testing.T) {
	...
	if err = pool.QueryRow(ctx, `INSERT INTO household(name) VALUES($1) RETURNING id`, fmt.Sprintf("Residual %d", stamp)).Scan(&household); err != nil {
		t.Fatal(err)
	}
	if err = pool.QueryRow(ctx, `INSERT INTO "user"(email,display_name,password_hash) VALUES($1,'Owner','unused') RETURNING id`, fmt.Sprintf("residual-%d@example.test", stamp)).Scan(&user); err != nil {
		t.Fatal(err)
	}
	...
	if err = pool.QueryRow(ctx, `INSERT INTO salary_source(household_id,user_id,employer,normalized_employer,is_primary) VALUES($1,$2,'Ordinary','ordinary',false) RETURNING id`, household, user).Scan(&ordinary); err != nil {
		t.Fatal(err)
	}
	insertSalary := func(sourceID, date, period string) string {
		...
		if err := pool.QueryRow(ctx, `INSERT INTO salary_event(salary_source_id,household_id,payroll_period,pay_date,net_pay,transaction_id,status,source_event_id) VALUES($1,$2,$3::date,$4::date,16000000,$5,'CONFIRMED',$6) RETURNING id`, sourceID, household, period, date, tx, source).Scan(&event); err != nil {
			t.Fatal(err)
		}
		return event
	}
	...
	p := New(pool)
	if err = p.Generate(ctx, Payload{HouseholdID: household, EndSalaryEventID: ordinaryEnd}); err != nil {
		t.Fatalf("stale/non-primary job: %v", err)
	}
	end := insertSalary(primary, "2026-09-25", "2026-09-01")
	errs := make(chan error, 4)
	for range 4 {
		go func() { errs <- p.Generate(ctx, Payload{HouseholdID: household, EndSalaryEventID: end}) }()
	}
	for range 4 {
		if err = <-errs; err != nil {
			t.Fatal(err)
	...
	if err = pool.QueryRow(ctx, `SELECT count(*) FROM review_item WHERE household_id=$1 AND review_type='CYCLE_RESIDUAL_ALLOCATION'`, household).Scan(&reviews); err != nil {
		t.Fatal(err)
	}
