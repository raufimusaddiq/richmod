...
func (p *Processor) agentStageTransferReview(ctx context.Context, state *agentState, call gateway.ToolCall, intent transferReconciliationIntent, candidateIDs []string, reason string) (agentToolResult, bool, error) {
	...
	if err = insertSourceEventReviewDecision(ctx, tx, state.HouseholdID, state.SourceEventID, "TRANSFER_CLASSIFICATION", map[string]any{"amount_idr": intent.amount, "transfer_relationship": reason}); err != nil {
		return result, true, err
	}
	var itemID string
