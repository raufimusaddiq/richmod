...
func (p *Processor) loadAgentReviewBinding(ctx context.Context, householdID string, update telegramUpdate) (*agentReviewBinding, any, int, error) {
	...
	rows, err := p.pool.Query(ctx, `
		...
		FROM (
			SELECT 'TRANSACTION'::text AS kind,r.id::text AS target_id,r.id::text AS review_request_id,
			       t.id::text AS transaction_id,r.review_type,COALESCE(c.state,''),COALESCE(t.merchant_id::text,''),
			       COALESCE(rr.telegram_message_id,0)::bigint,t.amount::text,COALESCE(t.counterparty_name,t.description,'Transaksi') AS label,
...
func (p *Processor) agentResolveBoundReview(ctx context.Context, state *agentState, call gateway.ToolCall, args map[string]any) (agentToolResult, bool, error) {
	...
	case "CYCLE_RESIDUAL":
		...
		if state.ReviewBinding.TelegramMessageID != 0 {
			bound.Update.Message.ReplyToMessage = &struct { MessageID int64 `json:"message_id"` }{MessageID: state.ReviewBinding.TelegramMessageID}
		}
...
func (p *Processor) agentResolveBoundWealthObservation(ctx context.Context, state *agentState, call gateway.ToolCall, args map[string]any, binding *agentReviewBinding) (agentToolResult, bool, error) {
	...
	case "SET_WEALTH_ACCOUNT":
		...
		if _, err = tx.Exec(ctx, `UPDATE wealth_observation SET resolved_wealth_account_id=$2,updated_at=now() WHERE id=$1 AND status='PENDING'`, binding.TargetID, id); err != nil { return result,true,err }
