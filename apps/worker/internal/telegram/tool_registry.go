...
func NativeFinanceTools() []gateway.ToolDefinition {
	...
	return []gateway.ToolDefinition{
		{Name: "query_transactions", Description: "Read household ledger transactions for a bounded period.", Parameters: objectSchema(map[string]any{"period": stringType, "search_text": map[string]any{"type": []string{"string", "null"}}}, []string{"period", "search_text"})},
...
func ValidateNativeToolCall(call gateway.ToolCall) (map[string]any, error) {
	allowed := map[string]bool{"query_transactions": true, "create_transaction": true, "propose_transaction_edit": true, "confirm_edit": true, "cancel_edit": true}
	if !allowed[call.Name] { return nil, fmt.Errorf("unregistered finance tool %q", call.Name) }
	var args map[string]any
	decoder := json.NewDecoder(bytes.NewReader(call.Arguments)); decoder.DisallowUnknownFields()
	if err := decoder.Decode(&args); err != nil || args == nil { return nil, fmt.Errorf("invalid arguments for %s", call.Name) }
	return args, nil
}
