...
func (p *Processor) tryJudgmentFastPath(ctx context.Context, sourceID, householdID string, update telegramUpdate, text string, now time.Time, state agentContextState) (bool, error) {
	...
	if answer.Choice == "NEEDS_GENERATIVE_AGENT" || answer.Choice == "SEARCH_TRANSACTIONS" || answer.Choice == "CREATE_TRANSACTION" || answer.Choice == "CREATE_TRANSFER" || answer.Choice == "CORRECT_TRANSACTION" || answer.Choice == "SALARY_INTERACTION" || answer.Choice == "MERCHANT_LEARNING_INTERACTION" || answer.Choice == "FINANCE_HELP" {
		return false, nil
	}
