...
func (p *Processor) agentRecordTransaction(ctx context.Context, state *agentState, call gateway.ToolCall, args map[string]any, metadata gateway.Metadata) (agentToolResult, bool, error) {
	...
	// The conversational tool runs only after the first bounded route pass. A
	// decisive native extraction therefore does not need a second Jev approval.
	// Jev is retained as a rescue lane when the generative result is partial or
	// low-confidence. Learned category shortcuts are bound to THIS merchant, not
	// merely to any household merchant that happens to use the same category.
	allowedCategories, _ := p.categorySlugs(ctx, state.HouseholdID)
	exactCategory := false
	if value.CategorySlug != "" && strings.TrimSpace(value.Merchant) != "" {
		if err = tx.QueryRow(ctx, `SELECT EXISTS(
			SELECT 1 FROM merchant_alias ma
			...
			WHERE ma.household_id=$1
			  ...
			  AND c.slug=$3
		)`, state.HouseholdID, value.Merchant, value.CategorySlug).Scan(&exactCategory); err != nil {
			return result, true, err
		}
	}
