...
func TestTelegramExpenseReviewReclassifiesAssetPurchase(t *testing.T) {
	url := os.Getenv("TEST_DATABASE_URL")
	if url == "" {
		t.Skip("TEST_DATABASE_URL is not configured")
	}
	...
	var household, userID, wealthID, transactionID, reviewID, replySource string
	must(pool.QueryRow(ctx, `INSERT INTO household(name) VALUES($1) RETURNING id`, fmt.Sprintf("telegram expense asset %d", stamp)).Scan(&household))
	must(pool.QueryRow(ctx, `INSERT INTO "user"(email,display_name,password_hash) VALUES($1,'Owner','unused') RETURNING id`, fmt.Sprintf("telegram-expense-asset-%d@example.test", stamp)).Scan(&userID))
	_, err = pool.Exec(ctx, `INSERT INTO household_member(household_id,user_id,role) VALUES($1,$2,'OWNER')`, household, userID)
	must(err)
	_, err = pool.Exec(ctx, `INSERT INTO telegram_identity(telegram_user_id,household_id,user_id) VALUES($1,$2,$3)`, chatID, household, userID)
	must(err)
	...
	must(err)
	markup := reviewActionMarkupPage(ctx, tx, reviewID, "AMBIGUOUS_CATEGORY", 0)
	must(tx.Rollback(ctx))
	if markup == nil || !strings.Contains(fmt.Sprint(markup), "Beli aset") || strings.Contains(fmt.Sprint(markup), "Ubah detail") {
		t.Fatalf("expense review markup=%+v", markup)
	}
	must(pool.QueryRow(ctx, `INSERT INTO source_event(household_id,source_type,external_id,received_at,payload_hash,processing_status) VALUES($1,'TELEGRAM_TEXT',$2,now(),$3,'RECEIVED') RETURNING id`, household, fmt.Sprintf("reply-%d", stamp), []byte(fmt.Sprintf("reply-%d", stamp))).Scan(&replySource))
	update := telegramUpdate{}
	update.Message.From.ID, update.Message.Chat.ID, update.Message.MessageID, update.Message.Text = chatID, chatID, 1, "Emas"
