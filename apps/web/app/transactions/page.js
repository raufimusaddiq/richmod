...
export default function TransactionsPage() {
  ...
  const load = useCallback(async (search = window.location.search) => {
    setLoading(true);
    try {
      const [transactions, categoryResponse, accountResponse, wealthAccountResponse, memberResponse] = await Promise.all([fetch(`/api/v1/transactions${search}`), fetch("/api/v1/categories"), fetch("/api/v1/accounts"), fetch("/api/v1/wealth/accounts"), fetch("/api/v1/household/members")]);
