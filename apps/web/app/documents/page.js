...
export default function DocumentsPage() {
  ...
  return <AppShell user={user} eyebrow="DOKUMEN" title="Bukti keuangan" actions={<span className="header-meta">JPEG / PNG · maks. 10 MB per gambar · maks. 10 gambar</span>}>
    <section className="surface upload-card"><div><span className="eyebrow">GENERIC PIPELINE</span><h2>Unggah bukti, sistem akan mengenalinya</h2><p>Beberapa gambar dalam satu kiriman diproses sebagai satu dokumen berurutan.</p></div><form onSubmit={upload}><input name="file" type="file" multiple accept="image/jpeg,image/png,.jpg,.jpeg,.png" required/><button disabled={working}>{working ? "Memproses…" : "Unggah & klasifikasikan"}</button></form></section>
