...
export default function Home() {
  ...
  return <AppShell user={user} eyebrow="RINGKASAN" title={`Keuangan keluarga · ${periodLabel}`} actions={<Link className="button secondary" href="/documents">＋ Unggah dokumen</Link>}>
    ...
    {overview?.reviewCount > 0 && <Link className="review-alert" href="/reviews"><span>!</span><div><b>{overview.reviewCount} transaksi butuh bantuanmu</b><small>Selesaikan keputusan agar buku keuangan tetap akurat.</small></div><strong>Buka Perlu Ditinjau →</strong></Link>}
