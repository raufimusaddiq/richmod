...
function WealthAccountList({ title, total, items, accountMap, liability = false }) {
  return <section className={`wealth-account-group ${liability ? "is-liability" : ""}`}>
    ...
    <div className="wealth-account-list">{items.map(item => <div className="wealth-account-row" key={item.wealthAccountId}>
      <div><b>{item.name}</b><small>{accountMeta(item, accountMap) || "Akun kekayaan"}</small></div><strong>{money(item.valueIdr)}</strong>
    </div>)}{!items.length && <p className="empty compact">Belum ada {liability ? "kewajiban" : "aset"} pada catatan ini.</p>}</div>
  </section>;
}
...
export default function WealthPage() {
  ...
  async function saveSnapshot(event) {
    ...
    if (!response.ok) setError("Catatan posisi belum dapat disimpan."); else { setSnapshotOpen(false); window.history.replaceState({}, "", "/wealth"); await load(); }
    setWorking(false);
  }
  ...
  async function openSnapshot(id) {
    ...
    if (!response.ok) { setError("Detail catatan posisi belum dapat dimuat."); return; }
  ...
  async function correctSnapshot(event) {
    ...
    if (!response.ok) setError("Koreksi catatan posisi belum dapat disimpan."); else { closeSnapshot(); await load(); }
    setWorking(false);
  }
  ...
  return <AppShell user={user} eyebrow="KEKAYAAN" title="Kekayaan bersih rumah tangga" actions={<a className="button secondary" href="/settings#wealth-accounts">Kelola akun kekayaan</a>}>
    <div className="wealth-page">
      <ErrorNotice message={error} retry={load}/>
      {loading ? <Skeleton cards={3} rows={3}/> : <>
        {!latest ? <section className="surface wealth-empty"><span className="eyebrow">MULAI DARI POSISI SEKARANG</span><h2>Kekayaan belum diatur</h2><p>Tambahkan semua akun aset dan kewajiban, lalu simpan satu catatan lengkap untuk mulai melihat kekayaan bersih dan perubahannya.</p><a className="button secondary" href="/settings#wealth-accounts">Siapkan akun kekayaan</a></section> : <>
          <section className="surface wealth-position">
            <div className="wealth-position-primary"><span className="eyebrow">POSISI TERKINI</span><h2>Kekayaan bersih</h2><strong>{money(latest.netWorthIdr)}</strong><p>Diamati {dateTime(latest.observedAt)}</p></div>
            <div className="wealth-balance-summary">
              <article><span>Total aset</span><strong className="positive">{money(latest.assetTotalIdr)}</strong><small>{assets.length} akun pada catatan posisi</small></article>
              <article><span>Total kewajiban</span><strong className="negative">{money(latest.liabilityTotalIdr)}</strong><small>{liabilities.length} akun pada catatan posisi</small></article>
            </div>
          </section>

          {previous && <section className="surface wealth-change">
            <div className="section-title"><div><span className="eyebrow">SEJAK CATATAN SEBELUMNYA</span><h2>Apa yang mengubah kekayaan bersih</h2></div><strong className={`wealth-change-total ${amountTone(data.summary?.netWorthChangeIdr)}`}>{signedMoney(data.summary?.netWorthChangeIdr)}</strong></div>
            <div className="wealth-change-equation">
              ...
              <div><span>Arus kas terkonfirmasi</span><strong className={amountTone(data.summary?.confirmedCashflowIdr)}>{signedMoney(data.summary?.confirmedCashflowIdr)}</strong><small>Dari catatan transaksi rumah tangga</small></div><i aria-hidden="true">+</i>
          ...
          <div className="wealth-analysis-grid">
            <section className="surface wealth-composition">
              <div className="section-title"><div><span className="eyebrow">KOMPOSISI KEKAYAAN</span><h2>Di mana kekayaan tersimpan</h2></div></div>
            ...
            {data.snapshots.length > 1 && <section className="surface wealth-trend"><div className="section-title"><div><span className="eyebrow">RIWAYAT KEKAYAAN BERSIH</span><h2>Arah kekayaan dari waktu ke waktu</h2></div><span className="header-meta">{data.snapshots.length} catatan</span></div><NetWorthHistoryChart items={data.snapshots} height={280}/></section>}
          </div>
        </>}
        ...
        <section className="surface wealth-snapshot">
          ...
          {showSnapshotForm && <form key={latest?.id || "initial"} className="wealth-snapshot-form" onSubmit={saveSnapshot}>
            ...
            <div className="wealth-snapshot-accounts">{activeAccounts.map(item => <fieldset className="wealth-snapshot-account" key={item.id}><legend><span>{item.name}</span><small>{[item.institution, wealthTypeLabel[item.wealthType], usageRoleLabel[item.usageRole]].filter(Boolean).join(" · ")}</small></legend><label><span>Nilai saat ini</span><input name={`value-${item.id}`} inputMode="numeric" pattern="[0-9]+" placeholder="Nilai IDR" required defaultValue={previousValues.get(item.id) || ""}/></label><label><span>Catatan</span><input name={`note-${item.id}`} placeholder="Opsional"/></label></fieldset>)}{!data.accounts.length && <p className="empty compact">Belum ada akun kekayaan. Tambahkan di Pengaturan.</p>}</div>
