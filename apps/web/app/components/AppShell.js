...
export default function AppShell({ user, title, eyebrow, actions, children }) {
  ...
  useEffect(() => { fetch("/api/v1/integration-actions").then(response => response.ok ? response.json() : []).then(items => setActionCount(Array.isArray(items) ? items.length : 0)).catch(() => {}); }, [pathname]);
  ...
  return <div className="app-frame">
    <aside className="sidebar"><Link className="brand" href="/"><span>R</span><div>Richmod<small>Family Finance</small></div></Link><nav>{links.map(([href, label, icon]) => <Link key={href} href={href} className={pathname === href ? "active" : ""}><i>{navIcon(icon)}</i>{label}{href === "/actions" && actionCount > 0 && <b className="nav-badge">{actionCount}</b>}</Link>)}</nav><div className="sidebar-user"><span>{user?.displayName?.slice(0, 1) || "U"}</span><div><b>{user?.displayName}</b><small>GMT+7 · IDR</small></div><button aria-label="Keluar" onClick={logout}>↪</button></div></aside>
