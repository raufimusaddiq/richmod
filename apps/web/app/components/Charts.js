...
export function NetWorthHistoryChart({ items, height = 260 }) {
  ...
  return <div className="chart-wrap" role="img" aria-label="Riwayat kekayaan bersih" style={{ height }}><ResponsiveContainer width="100%" height="100%"><BarChart data={data} margin={{ top: 12, right: 12, left: 0, bottom: 0 }}><CartesianGrid stroke="#e8ece7" vertical={false}/><XAxis dataKey="label" axisLine={false} tickLine={false}/><YAxis hide/><Tooltip formatter={(_, __, item) => money(item.payload.raw)} contentStyle={tooltipStyle}/><Bar dataKey="value" fill="#5f8f72" radius={[4,4,0,0]} maxBarSize={42}/></BarChart></ResponsiveContainer></div>;
}
