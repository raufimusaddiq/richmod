...
export function DailyCashflowChart({ items, height = 300 }) {
  const data = items.map(item => ({ ...item, label: item.period?.slice(8) || "", expenseValue: Number(item.expense || 0), cumulativeValue: Number(item.cumulativeExpense || 0) }));
  if (!data.length) return <p className="empty compact">Belum ada data pada siklus aktif.</p>;
  return <div className="chart-wrap" style={{ height }}><ResponsiveContainer width="100%" height="100%"><BarChart data={data} margin={{ top: 10, right: 12, left: 0, bottom: 0 }} barCategoryGap="22%"><CartesianGrid stroke="#e8ece7" vertical={false}/><XAxis dataKey="label" axisLine={false} tickLine={false} tick={{ fill: "#6f786f", fontSize: 11 }} interval={data.length > 14 ? 2 : 0}/><YAxis hide/><Tooltip formatter={(value, name) => [money(String(Math.round(value))), { expenseValue: "Pengeluaran hari ini", cumulativeValue: "Total berjalan" }[name]]} labelFormatter={label => `Tanggal ${label}`} contentStyle={{ border: "1px solid #dfe6df", borderRadius: 12 }}/><Legend formatter={value => ({ expenseValue: "Pengeluaran hari ini", cumulativeValue: "Total berjalan" }[value])}/><Bar dataKey="expenseValue" fill="#d47a66" radius={[4,4,0,0]} maxBarSize={20}/><Line type="monotone" dataKey="cumulativeValue" stroke="#526b8b" strokeWidth={2.5} dot={false}/></BarChart></ResponsiveContainer></div>;
}
