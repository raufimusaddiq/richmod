...
func (h *Handler) CycleDaily(w http.ResponseWriter, r *http.Request) {
	...
	defer rows.Close(); result := make([]map[string]string, 0); spent := big.NewInt(0); cumulative := big.NewInt(0)
	for rows.Next() { var period, income, expense, net string; if err := rows.Scan(&period,&income,&expense,&net); err != nil { writeJSON(w,500,map[string]string{"error":"unable to calculate daily cycle"}); return }; amount, _ := new(big.Int).SetString(expense,10); spent.Add(spent,amount); cumulative.Add(cumulative,amount); result = append(result,map[string]string{"period":period,"income":income,"expense":expense,"netCashflow":net,"cumulativeExpense":cumulative.String()}) }
	days := int(end.Sub(*start).Hours()/24); elapsed := int(now.Sub(*start).Hours()/24)+1; if elapsed < 0 { elapsed = 0 }; if elapsed > days { elapsed = days }
	remaining := new(big.Int); salaryValue, ok := new(big.Int).SetString(salary,10); if !ok { salaryValue = big.NewInt(0) }; remaining.Sub(salaryValue,spent); if remaining.Sign() < 0 { remaining.SetInt64(0) }
	writeJSON(w, 200, map[string]any{"daily":result,"salary":salaryValue.String(),"spent":spent.String(),"remaining":remaining.String(),"daysElapsed":elapsed,"daysTotal":days,"cycleStart":start.Format("2006-01-02"),"cycleEnd":end.Format("2006-01-02")})
}
