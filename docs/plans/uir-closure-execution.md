...
**UIRC-02 A in progress:** `reviewdomain.ReconcileTransfer` now owns transfer-case locking, candidate revalidation, confirm/create/ignore, evidence, source/observation refresh, and canonical review/request completion. Web, native Telegram, and bound agent mutations delegate to it. Native Telegram requires a bound reply/card instead of selecting the newest household case. PostgreSQL parity/concurrency regression remains the exit gate.
**UIRC-02 B in progress:** `reviewdomain.ResolveFinancialEmailReview` now owns financial-email IGNORE plus partial/final entity binding: partial picks persist and re-ask, the final entity queues `PROCESS_FINANCIAL_EMAIL`, and the review/request/conversation transition once. Web and Telegram adapters own decoding, display, audit, and replies. CI parity test required before exit.
**UIRC-04 in progress:** Admin review surface counts now read the canonical resolution audit row instead of guessing from the resolver's Telegram identity, so a Telegram-linked user resolving from Web counts as WEB. Web Escape Rate denominator is Telegram-path resolutions only; Web-only reviews and voluntary wealth/details navigation are excluded. `TestReviewOpsAdminAggregatesAndRedaction` seeds both surfaces. CI run required before exit.
